...
public final class App {
    ...
    private static final String DYNAMODB_TABLE =
            System.getProperty("dynamodb.table", "ledger-sync");
        ...
        switch (args[0]) {
            ...
            case "report" -> {
                ...
                try (SqlLedgerStore store = new SqlLedgerStore(DB)) {
                    ...
                    Files.writeString(
                            ...
                            Json.writePretty(
                                    Reports.reconciliation(ledger, balanceEvidence)));
            ...
            case "backfill" -> {
                try (SqlLedgerStore sql = new SqlLedgerStore(DB);
                     ...
                     DynamoDbDocumentStore documents =
                             new DynamoDbDocumentStore(client, DYNAMODB_TABLE)) {

                    DynamoDbSetup.ensureTable(client, DYNAMODB_TABLE);
            ...
            case "consistency-check" -> {
                try (SqlLedgerStore sql = new SqlLedgerStore(DB);
                    ...
                    var divergences =
                            new ConsistencyChecker(sql, documents).check();

                    if (divergences.isEmpty()) {
                        System.out.println(
                                "OK: the document store agrees with SQL ("
                                        + sql.all().size()
                                        + " transactions checked)");
                    } else {
                        System.out.println(
                                divergences.size() + " divergence(s) found:");
   ...
   private static DynamoDbClient dynamoClient() {
    ...
    private static void saveBalanceEvidence(
            Map<String, BigDecimal> fresh) throws IOException {

        Map<String, BigDecimal> merged =
                new LinkedHashMap<>(loadBalanceEvidence());
        ...
        Files.writeString(
                BALANCE_EVIDENCE,
                Json.writePretty(out));
    }

    private static Map<String, BigDecimal> loadBalanceEvidence()
            throws IOException {

        if (!Files.exists(BALANCE_EVIDENCE)) {
            return Map.of();
        }

        Map<String, Object> raw =
                Json.parseObject(Files.readString(BALANCE_EVIDENCE));

        Map<String, BigDecimal> out = new LinkedHashMap<>();

        raw.forEach(
                (k, v) -> out.put(k, new BigDecimal((String) v)));
