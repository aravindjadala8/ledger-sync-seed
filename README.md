...
JDK 21 (only, for verify.sh â€” no network, no database, no Gradle needed)

For the full CLI (migrate / ingest / report / backfill /
consistency-check): a way to get the H2 JDBC driver onto the runtime
classpath â€” either Gradle resolving it from Maven Central, or the
Dockerfile's curl step (see Â§4). Neither of these was reachable in the
sandbox this was built in â€” see Â§10, "Environment limitations
encountered" â€” so those five commands are implemented and unit-tested
...
SelfCheck, which ingests fixtures/corpus-a.jsonl end-to-end (parse â†’
dedup â†’ categorize â†’ in-memory ledger) and prints the same category/account
...
The 4821/146-vs-145 gap is not a bug â€” see Â§8.

3. The full CLI (needs the H2 driver on the classpath â€” see Â§10)
...
against InMemoryDocumentStore (see Â§9) persisted to
...
Also includes a dynamodb-local service for the DynamoDB design in Â§9. See
...
Central there â€” Â§10). Please verify before relying on it.

5. Tests

./gradlew test

Could not be run in this sandbox â€” there is no committed Gradle wrapper
and no network path to Maven Central to fetch one or to resolve JUnit (see
Â§10). What was actually done instead, and is fully reproducible:
...
store, backfill, consistency checker â€” full list in Â§11), using real
...
code â€” the shim only stands in for the dependency this sandbox can't
download.

Result, last run: 59/59 passed, including the two frozen
NormalizedTxnContractTest/AmountsTest files (extended, not modified,
in AmountsTest's case â€” one whole-rupee regression case added).

6. Verification

./verify.sh (Â§2) is the always-available check. SelfCheck.java compares
the ingest result against fixtures/corpus-a-totals.json and prints exactly
where it does and doesn't match â€” see Â§8 for what the one remaining gap is
and why it's correct, not a bug.

7. Incident: INC-2026-09-11 (the â‚¹92,213.10 water can)

Root cause. Amounts.first(String) matched rupee figures with the
pattern (?:Rs\.?|INR)\s*([0-9,]+\.[0-9]{2}) â€” exactly two decimal
...
Amounts's regex now makes the fractional part optional:
([0-9,]+(?:\.[0-9]{1,2})?).

Structural fix, not just a wider regex: HdfcSmsParser,
...
message â€” this holds regardless of what other numbers appear in the
...
production code â€” see the decision log (Â§13, entries 1â€“2).

Walking account 4821's balance chain in true occurred-time order (opening
â‚¹48,211.40 â†’ closing â‚¹41,126.34), using only messages that exist in the
corpus, breaks in exactly two places:

2026-07-22, +â‚¹649.00 â€” a NETFLIX debit evidenced only by an email
(no SMS exists for it at all). This is captured â€” it becomes
ledger transaction #145.

2026-07-29, between 11:53 and 17:06, +â‚¹7,500.00 â€” m-00203-bbc8db
(11:53, balance â‚¹36,054.05) is the last point where the chain matches;
m-00204-4741e7 (17:06, balance â‚¹28,479.05, amount â‚¹75.00) is the next.
36,054.05 âˆ’ 7,500.00 âˆ’ 75.00 = 28,479.05 exactly. No message of any
kind â€” SMS or email â€” evidences this debit. A full-text search of the
...
sourceMessageIds â€” there is no way to construct a 146th ledger row without
...
checkpoint total (91) matches exactly â€” this both confirms the balance-chain
...
ledger/bank balances, and the â‚¹7,500.00 unaccounted amount â€” see
...
duplicates â€” this is what makes Backfill reruns safe.
...
domains only) â€” writing an AWS SDK v2 client here would mean shipping
...
on every write) and the exact same idempotent-write semantics â€” and is
...
docker-compose.yml's dynamodb-local service) is the concrete next step â€”
listed as UNFINISHED in Â§15, not claimed as done.
...
Q1 â€” one account's month, newest first

2,116

2,116

Q2 â€” category running totals for an account

4

4

Q3 â€” transaction for a given message id
...
handful of language-package registries (npm, pip, crates, GitHub) â€” not
...
SDK) could not be exercised. Â§5 explains exactly what was done instead
...
against InMemoryLedgerStore â€” this is also just better dependency
...
11. Test suite (59 methods, 12 classes â€” all run via the shim, Â§5)
...
content-derived key used everywhere â€” never messageId/receivedAt),
TxnGroup (mutable scratch space for one transaction being assembled),
Deduplicator, Categorizer.

13. Decision log

Transaction identity excludes messageId/receivedAt entirely.
...
1:1 approach) â€” explicitly the thing the assignment warns against.

The 4821 145-vs-146 gap is reported, not forced. Investigated via a
...
dishonest â€” see Â§8). Alternative rejected: treating it as a dedup bug and
widening the identity key's tolerance until 146 came out â€” this would
have silently merged unrelated transactions on the hidden corpus.

TRANSFER evidence is a matched opposite leg, not a merchant-string
...
opposite leg anywhere in the corpus â€” regex-matching IMPS/P2A would
...
same 5 pairs, zero extra matches â€” the window bounds the search, it
doesn't loosen the match.

MICRO requires the UPI-merchant prefix, not just amount â‰¤ â‚¹100.
Evidence: filtering the corpus for "debit, UPI-prefixed merchant, amount
â‰¤ 100" reproduces the checkpoint's micro_count (52/45) exactly; several
non-UPI purchases â‰¤ â‚¹100 (IRCTC 99.99, DMART 99.99, SWIGGY 47.33, ...)
are correctly excluded from that count. Amount alone would have
overcounted MICRO by roughly 30 transactions.

Amount extraction moved from a whole-body scan into each format's own
regex. See Â§7. Rejected alternative: just widening Amounts.first's
regex and leaving parsers calling it against the raw body â€” this would
...
A small non-frozen "balance evidence" side-file, not a schema change,
...
Backfill/ConsistencyChecker depend on the LedgerStore interface,
not SqlLedgerStore. Forced by this sandbox's inability to construct
a working SqlLedgerStore at all (no H2 driver reachable â€” Â§10), but a
...
No framework. The seed already avoids one; nothing implemented here
...
DynamoDB (design) over MongoDB, even though only an in-memory stand-in
...
Rejected: MongoDB â€” would work too, but its extra query flexibility is
unused here and its compound-index design is more to get precisely right
for the exact same three patterns.

InMemoryDocumentStore persists to a flat JSON file between CLI
...
legitimate SMS/email pair for the same real transaction (â‚¹2,750.00, APOLLO
PHARMACY, account 9075) 9 days apart â€” the only SMS and the only email
...
(needed separately, since IRCTC ~â‚¹99.99 and similar amounts do recur on the
...
PK/SK/GSI design in Â§9) â€” not written. InMemoryDocumentStore implements
...
assistant and were not attempted â€” they need the actual Simplify Money
app, real friends, and real screenshots from you.

Five-minute walkthrough video â€” not recorded (needs your voice/screen).

The 4821 "145 vs 146" situation (Â§8) is not unfinished â€” it's the
intended, investigated, documented outcome â€” but is listed here too so it
...
that width â€” but it is still a bound, and a hidden corpus with two
...
(Â§14), so it depends on the merchant-normalization key holding across
...
PS C:\Users\91986\Downloads\ledger-sync-seed>